...
def conv_block(input_tensor, kernel_size, filters, stage, block, strides=(2, 2)):
    ...
    x = Conv2D(filters1, (1, 1), strides=strides,kernel_initializer=random_normal(stddev=0.02),name=conv_name_base + '2a', use_bias=False)(input_tensor)
    x = BatchNormalization(name=bn_name_base + '2a')(x)
    x = Activation('relu')(x)
...
def df_block(input):
    ...
    #result=input+diff_img
    return diff_img

def df_block_shape(input_shape):
    return (input_shape[0],input_shape[1],input_shape[2],input_shape[3])

def res_df_block(input_tensor,filter1,filter2,filter3):
    x=Conv2D(filter1,(1,1),padding='same',kernel_initializer=random_normal(stddev=0.02),use_bias=False)(input_tensor)
    x=BatchNormalization()(x)
    x=Activation('relu')(x)

    df=Lambda(df_block,df_block_shape)(x)
    
    y=Conv2D(filter2,(3,3),padding='same',kernel_initializer=random_normal(stddev=0.02),use_bias=False)(df)
    y=BatchNormalization()(y)
    y=Activation('relu')(y)

    y=Conv2D(filter3,(1,1),padding='same',kernel_initializer=random_normal(stddev=0.02),use_bias=False)(y)
    y=BatchNormalization()(y)
    
    z=layers.add([y,input_tensor])
    z=Activation('relu')(z)
    return z
...
def ResNet50(inputs):
    ...
    #x=Lambda(df_block,df_block_shape)(x)
    ...
    x = res_df_block(x,64,64,256)
    # x = identity_block(x, 3, [64, 64, 256], stage=2, block='b')
    x = identity_block(x, 3, [64, 64, 256], stage=2, block='c')

    # 128,128,256 -> 64,64,512
    x = conv_block(x, 3, [128, 128, 512], stage=3, block='a')
    x=res_df_block(x,128,128,512)
    #x=res_df_block(x,128,128,512)
